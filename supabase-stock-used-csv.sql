-- ===================================================================
-- "Stock Used" column for Shop 1-4 / Kitchen monthly CSVs, plus a fix
-- for archive_and_clear_daily().
--
-- Background: archive_and_clear_daily() was first defined in
-- supabase-daily-archive.sql (log archiving + clearing only), then
-- REDEFINED in supabase-monthly-sheets.sql to also record
-- daily_item_activity (the per-day Added/Subtracted data the Monthly
-- Inventory Sheets CSV is built from). The Closing Stock migration
-- later re-ran supabase-daily-archive.sql's ORIGINAL definition (to add
-- "delete from closing_stock_locks"), which overwrote the
-- monthly-sheets version and silently stopped the daily_item_activity
-- recording.
--
-- This script:
--   1. Creates daily_closing_stock, which persists each day's "Stock
--      Used" values (from closing_stock_locks) before they're cleared.
--   2. Redefines archive_and_clear_daily() with BOTH the
--      daily_item_activity recording (restoring the lost monthly-sheets
--      behaviour) AND the closing-stock handling.
--   3. Redefines snapshot_end_of_day() to add 60-day retention for
--      daily_closing_stock, matching the other daily_* tables.
--
-- Run this once in the Supabase SQL Editor.
-- ===================================================================

-- 1. Persisted daily "Stock Used" values (Shop 1-4 / Kitchen only —
--    Warehouse never writes to closing_stock_locks).
...
alter table daily_closing_stock enable row level security;
create policy "anon full access" on daily_closing_stock for all using (true) with check (true);

-- 2. 5pm IST job: archive + clear logs/request queues, record TODAY's
--    activity so far into daily_item_activity (split by source — same
--    classification as classifyLogEntry() in app.js), persist today's
--    Closing Stock "Stock Used" values into daily_closing_stock, then
--    clear logs/requests/closing_stock_locks. The evening (5pm-to-midnight)
--    portion of today gets added to daily_item_activity on top by
--    snapshot_end_of_day() at midnight.
...
begin
  ...
  -- Persist today's "Stock Used" values (from Closing Stock updates)
  -- before closing_stock_locks is cleared below. Rows with stock_used
  -- null (locked via "Update Stock" without a closing-stock action) are
  -- not meaningful "Stock Used" values, so they're skipped.
  insert into daily_closing_stock (location, activity_date, item_name, stock_used)
  select location, today, item_name, stock_used
...
-- 3. Midnight IST job: finalize YESTERDAY's activity by adding the
--    5pm-to-midnight portion, snapshot end-of-day stock totals, and prune
--    all daily_* tables down to a 60-day retention window.
